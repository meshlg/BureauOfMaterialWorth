...
local strings = {
    ...
    [SI_BMW_WINDOW_VERSION_DATE] = "Версия аддона 3.0.060753 от 22.07.2026",
    ...
    [SI_BMW_DETAIL_CUM_TOOLTIP_BODY] = "Доля каждого материала в общей стоимости этого списка, считается от самого дорогого вниз - поэтому число не меняется, как бы вы ни сортировали таблицу. Читайте его на виде |cFFF897по стоимости|r (по умолчанию): строки примерно до 80% - это те немногие стаки, что дают основную ценность, их и вези на продажу в первую очередь, остальное можно не трогать. Итоговые 100% всегда приходятся на самый дешёвый материал. Материалы без цены не учитываются и показывают прочерк.",
    ...
    [SI_BMW_ROW_TOOLTIP_TOTAL] = "Стоимость стаки: %s",
    ...
    [SI_BMW_DETAIL_BTN_CHANGES_TOOLTIP_BODY] = "Показать, как изменилась ремесленная сумка с момента сохранённого снимка: какие материалы добавились, убыли или изменились в количестве, и стоимость каждого движения в золоте. Базовый снимок один раз создаётся автоматически при первом открытии непустой сумки; «Запомнить» можно нажать в любой момент, чтобы заменить его.",
    ...
    [SI_BMW_FOOTER_COVERAGE_LABEL] = "Покрытие цен",
    ...
    [SI_BMW_FOOTER_GUIDANCE_CHANGES] = "Со снимка: %s%s золота - открыть изменения",
